	...
	public static void attackPlayer( Client c, int i )
	{
		if( PlayerHandler.players[i] != null ) {
			...
			if( PlayerHandler.players[i] != null ) {
				...
				if( ! c.goodDistance( PlayerHandler.players[i].getX(), PlayerHandler.players[i].getY(),
						c.getX(), c.getY(), 25 )
						&& ! sameSpot ) {
					c.getCombat().resetPlayerAttack();
					return;
				...
				if( c.attackTimer <= 0 ) {
					...
					if( ! c.goodDistance( c.getX(), c.getY(), PlayerHandler.players[i].getX(),
							...
							|| ! c.goodDistance( c.getX(), c.getY(), PlayerHandler.players[i].getX(),
									PlayerHandler.players[i].getY(), c.getCombat().getRequiredDistance() )
					...
					if( c.getCombat().correctBowAndArrows() < c.playerEquipment[c.playerArrows]
							...
							&& c.playerEquipment[c.playerWeapon] != 9185 && ! c.usingMagic ) {
					...
					if( usingOtherRangeWeapons ) { // knives, darts, etc hit
													// delay
						c.rangeItemUsed = c.playerEquipment[c.playerWeapon];
					...
					if( c.usingMagic ) { // magic hit delay
						...
						if( c.MAGIC_SPELLS[c.spellId][4] > 0 ) {
							c.getPA().createPlayersProjectile( pX, pY, offX, offY, 50, 78,
									c.MAGIC_SPELLS[c.spellId][4], c.getCombat().getStartHeight(),
					...
					if( usingBow && Config.CRYSTAL_BOW_DEGRADES ) { // crystal
																	// bow
																	// degrading
