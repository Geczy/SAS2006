...
public class MagicOnFloorItems implements PacketType
{
	...
	public void processPacket( final Client c, int packetType, int packetSize )
	{
		 ...
		 * Telegrab spell
		 */
		if( ( c.getItems().freeSlots() >= 1 || c.getItems().playerHasItem( itemId, 1 ) ) && Item.itemStackable[itemId]
			...
			if( c.goodDistance( c.getX(), c.getY(), itemX, itemY, 12 ) ) {
				...
				c.getPA().createPlayersProjectile( c.getX(), c.getY(), offX, offY, 50, 70, c.MAGIC_SPELLS[51][4], 50,
						10, 0, 50 );
		...
		if( c.goodDistance( c.getX(), c.getY(), itemX, itemY, 12 ) ) {
			...
			c.gfx100( c.MAGIC_SPELLS[51][3] );
			c.getPA().createPlayersStillGfx( 144, itemX, itemY, 0, 72 );
			c.getPA().createPlayersProjectile( c.getX(), c.getY(), offX, offY, 50, 70, c.MAGIC_SPELLS[51][4], 50, 10,
					0, 50 );
			c.getPA().addSkillXP( c.MAGIC_SPELLS[51][7], 6 );
			c.getPA().refreshSkill( 6 );
			c.stopMovement();
