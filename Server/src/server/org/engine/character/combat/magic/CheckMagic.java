...
public class CheckMagic
{
	...
	public static boolean checkMagicReqs( Client c, int spell )
	{
		if( c.usingMagic && Config.RUNES_REQUIRED ) { // check for runes
			if( ! c.getItems().playerHasItem( c.MAGIC_SPELLS[spell][8], c.MAGIC_SPELLS[spell][9] ) && ! c.getCombat()
					.wearingStaff( c.MAGIC_SPELLS[spell][8] )
					||
					! c.getItems().playerHasItem( c.MAGIC_SPELLS[spell][10], c.MAGIC_SPELLS[spell][11] ) && ! c
							.getCombat().wearingStaff( c.MAGIC_SPELLS[spell][10] )
					||
					! c.getItems().playerHasItem( c.MAGIC_SPELLS[spell][12], c.MAGIC_SPELLS[spell][13] ) && ! c
							.getCombat().wearingStaff( c.MAGIC_SPELLS[spell][12] )
					||
					! c.getItems().playerHasItem( c.MAGIC_SPELLS[spell][14], c.MAGIC_SPELLS[spell][15] ) && ! c
							.getCombat().wearingStaff( c.MAGIC_SPELLS[spell][14] ) ) {
		...
		if( c.usingMagic && staffRequired > 0 && Config.RUNES_REQUIRED ) { // staff
			...
			if( c.playerEquipment[c.playerWeapon] != staffRequired ) {
				c.sendMessage( "You need a " + c.getItems().getItemName( staffRequired ).toLowerCase()
						+ " to cast this spell." );
				return false;
			}
		}
