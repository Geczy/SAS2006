			...
			case FIRST_CLICK:
				...
				if( c.playerRights >= 3 ) {
					...
					Misc.println( "objectId: " + c.objectId + "  ObjectX: " + c.objectX + "  objectY: " + c.objectY
							...
							+ " Distance: " + c.objectDistance );
				}
				if( Math.abs( c.getX() - c.objectX ) > 25 || Math.abs( c.getY() - c.objectY ) > 25 ) {
					c.resetWalkingQueue();
					break;
				}
				for( int element: c.getRunecrafting().altarID ) {
					if( c.objectId == element ) {
				...
				switch( c.objectId ) {
					...
					case 9398:// deposit
						...
						c.getPA().sendFrame248( 4465, 197 );// 197 just because
															// you can't see it
															// =\
					...
					CycleEventHandler.getSingleton().addEvent( c, new CycleEvent()
					{
						...
						public void execute( CycleEventContainer container )
						{
							if( c.clickObjectType == 1
									&& c.goodDistance( c.objectX + c.objectXOffset, c.objectY + c.objectYOffset,
