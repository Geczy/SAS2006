...
public class ActionHandler
{
	...
	public void firstClickObject( int objectType, int obX, int obY )
	{
		...
		Misc.println( "objectId: " + c.objectId + "  ObjectX: " + c.objectX + "  objectY: " + c.objectY + " Xoff: "
				+ ( c.getX() - c.objectX ) + " Yoff: " + ( c.getY() - c.objectY ) + " Distance: " + c.objectDistance );
		...
		switch( objectType ) {
			...
			case 8959:
				if( c.getX() == 2490 && ( c.getY() == 10146 || c.getY() == 10148 ) ) {
					if( c.getPA().checkForPlayer( 2490, c.getY() == 10146 ? 10148: 10146 ) ) {
						new Object( 6951, c.objectX, c.objectY, Player.heightLevel, 1, 10, 8959, 15 );
					}
				}
				break;
			...
			case 10284:
				...
				if( c.barrowsNpcs[c.randomCoffin][1] == 0 && c.barrowsKillCount >= 5 ) {
					Server.npcHandler.spawnNpc( c, c.barrowsNpcs[c.randomCoffin][0], 3551, 9694 - 1, 0, 0, 120, 30,
							200, 200, true, true );
	...
	public void secondClickNpc( int npcType )
	{
		...
		switch( npcType ) {
			...
			case 519:
				c.getShops().openShop( 8 ); // should sell barrows or something
											// like that
				break;
