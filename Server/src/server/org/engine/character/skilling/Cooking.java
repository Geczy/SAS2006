...
public class Cooking extends SkillHandler
{
	...
	public static void cookThisFood( Client p, int i, int object )
	{
		switch( i ) {
			...
			case 321:
				...
				break;
			case 327:
				cookFish( p, i, 40, 1, 369, 325, object );
				break;
			case 345:
			...
			case 361:
				...
				break;
			case 363:
				cookFish( p, i, 120, 46, 367, 365, object );
				break;
			case 377:
	...
	private static void cookFish( Client c, int itemID, int xpRecieved, int levelRequired, int burntFish,
			int cookedFish, int object )
	{
	...
	private static void cookTheFish( final Client c )
	{
		...
		CycleEventHandler.getSingleton().addEvent( c, new CycleEvent()
		{
			...
			{
				c.getItems().deleteItem( c.playerSkillProp[7][0], c.getItems().getItemSlot( c.playerSkillProp[7][0] ),
						1 );
				if( c.playerLevel[7] >= fishStopsBurning( c.playerSkillProp[7][0] )
					...
					c.sendMessage( "You successfully cook the "
							+ c.getItems().getItemName( c.playerSkillProp[7][0] ).toLowerCase() + "." );
