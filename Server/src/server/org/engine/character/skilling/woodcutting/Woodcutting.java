...
public class Woodcutting extends SkillHandler
{
	...
	private static boolean hasAxe( Client c )
	{
		boolean has = false;
		for( int[] axe: axes ) {
			if( c.getItems().playerHasItem( axe[0] ) || c.playerEquipment[3] == axe[0] ) {
				has = true;
			}
		}
		return has;
	...
	private static int getAxeTime( Client c )
	{
		int axe = - 1;
		for( int[] axe2: axes ) {
			if( c.playerLevel[8] >= axe2[1] ) {
				if( c.getItems().playerHasItem( axe2[0] ) || c.playerEquipment[3] == axe2[0] ) {
					axe = axe2[3];
				}
			}
		}
