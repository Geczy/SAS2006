...
public class PlayerAssistant
{
	...
	public static void QuestReward( Client c, String questName, int PointsGain, String Line1, String Line2,
			String Line3, String Line4, String Line5, String Line6, int itemID )
	{
				...
				for( int i2 = 1; i2 < Config.MAX_PLAYERS; i2 ++ ) {
					...
					if( p != null && p.isActive && Misc.playerNameToInt64( p.playerName ) == friend ) {
						...
						if( o != null ) {
							if( c.playerRights >= 2
									|| p.privateChat == 0
									|| p.privateChat == 1 && o.getPA()
											.isInPM( Misc.playerNameToInt64( c.playerName ) ) ) {
								loadPM( friend, 1 );
								pmLoaded = true;
							}
							break;
	...
	public void giveLife()
	{
		...
		c.faceUpdate( - 1 );
		c.freezeTimer = 0;
		if( c.duelStatus <= 4 && ! c.getPA().inPitsWait() ) { // if we are not
																// in a duel we
																// must be in
																// wildy so
																// remove items
	...
	private final int randomHolidayItem = Misc.random( 15 );
	...
	public void fixAllBarrows()
	{
		...
		for( int j = 0; j < c.playerItems.length; j ++ ) {
			...
			for( int[] brokenBarrow: c.getItems().brokenBarrows ) {
