...
public abstract class Player
{
	...
	public int specMaxHitIncrease, freezeDelay, freezeTimer = - 6, killerId, playerIndex, oldPlayerIndex,
			...
			teleHeight, teleX, teleY, rangeItemUsed, killingNpcIndex, totalDamageDealt, oldNpcIndex, fightMode,
	...
	public void updateThisPlayerMovement( Stream str )
	{
		...
		if( dir1 == - 1 ) {
			...
			if( dir2 == - 1 ) {
				...
				if( playerEnergy > 0 && mode == 1 || mode == 2 || mode == 3 ) {
					playerEnergy -= 1;
				} else if( playerEnergy > 0 && mode == 4 || mode == 5 ) {
					playerEnergy = 100;
				}
				else {
					isRunning2 = false;
	 ...
	 * //playerProps.writeByte(bountyIcon); if (playerEquipment[playerHat] > 1)
	 * { playerProps.writeWord(0x200 + playerEquipment[playerHat]); } else {
	 * playerProps.writeByte(0); } if (playerEquipment[playerCape] > 1) {
	 * playerProps.writeWord(0x200 + playerEquipment[playerCape]); } else {
	 * playerProps.writeByte(0); } if (playerEquipment[playerAmulet] > 1) {
	 * playerProps.writeWord(0x200 + playerEquipment[playerAmulet]); } else {
	 * playerProps.writeByte(0); } if (playerEquipment[playerWeapon] > 1) {
	 * playerProps.writeWord(0x200 + playerEquipment[playerWeapon]); } else {
	 * playerProps.writeByte(0); } if (playerEquipment[playerChest] > 1) {
	 * playerProps.writeWord(0x200 + playerEquipment[playerChest]); } else {
	 * playerProps.writeWord(0x100+playerAppearance[2]); } if
	 * (playerEquipment[playerShield] > 1) { playerProps.writeWord(0x200 +
	 * playerEquipment[playerShield]); } else { playerProps.writeByte(0); } if
	 * (!Item.isFullBody(playerEquipment[playerChest])) {
	 * playerProps.writeWord(0x100+playerAppearance[3]); } else {
	 * playerProps.writeByte(0); } if (playerEquipment[playerLegs] > 1) {
	 * playerProps.writeWord(0x200 + playerEquipment[playerLegs]); } else {
	 * playerProps.writeWord(0x100+playerAppearance[5]); } if
	 * (!Item.isFullHelm(playerEquipment[playerHat]) &&
	 * !Item.isFullMask(playerEquipment[playerHat])) {
	 * playerProps.writeWord(0x100 + playerAppearance[1]); } else {
	 * playerProps.writeByte(0); } if (playerEquipment[playerHands] > 1) {
	 * playerProps.writeWord(0x200 + playerEquipment[playerHands]); } else {
	 * playerProps.writeWord(0x100+playerAppearance[4]); } if
	 * (playerEquipment[playerFeet] > 1) { playerProps.writeWord(0x200 +
	 * playerEquipment[playerFeet]); } else {
	 * playerProps.writeWord(0x100+playerAppearance[6]); } if
	 * (playerAppearance[0] != 1 &&
	 * !Item.isFullMask(playerEquipment[playerHat])) {
	 * playerProps.writeWord(0x100 + playerAppearance[7]); } else {
	 * playerProps.writeByte(0); } playerProps.writeByte(playerAppearance[8]);
	 ...
	 * playerProps.writeQWord(Misc.playerNameToInt64(playerName)); int mag =
	 * (int) ((getLevelForXP(playerXP[6])) * 1.5); int ran = (int)
