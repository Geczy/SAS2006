...
public class MithrilSeeds
{
	...
	public static void plant( final Client c )
	{
		...
		for( Objects o: ObjectHandler.globalObjects ) {
			if( Objects.getObjectX() == c.getX() && Objects.getObjectY() == c.getY() && Objects.objectId != - 1 ) {
		...
		if( System.currentTimeMillis() - FlowerDelay > 1600 ) {
			...
			c.getPA().walkTo( - 1, 0 );
			if( Region.getClipping( c.getX() - 1, c.getY(), Player.heightLevel, - 1, 0 ) ) {
				c.getPA().walkTo( - 1, 0 );
			} else if( Region.getClipping( c.getX() + 1, c.getY(), Player.heightLevel, 1, 0 ) ) {
				c.getPA().walkTo( 1, 0 );
			} else if( Region.getClipping( c.getX(), c.getY() - 1, Player.heightLevel, 0, - 1 ) ) {
				c.getPA().walkTo( 0, - 1 );
			} else if( Region.getClipping( c.getX(), c.getY() + 1, Player.heightLevel, 0, 1 ) ) {
				c.getPA().walkTo( 0, 1 );
			}
