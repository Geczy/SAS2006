...
public class lottery {
	...
	public static ArrayList<String> lotteryPlayersNames = new ArrayList<String> (); //players (playername)
	public static ArrayList<String> unclaimedWinners = new ArrayList<String> (); //Winners that havent claimed
	...
	public static void Process(){
		if (System.currentTimeMillis() - Server.lottery.lastAnnouncment > (1000 * 60 * Server.lottery.announcmentFrequency)) {
			announceFund();
			lastAnnouncment = System.currentTimeMillis();
		}
		if (System.currentTimeMillis() - Server.lottery.nextDraw > (1000 * 60 * Server.lottery.nextDrawFrequency) && lotteryFund >= 4000) {
			drawLottery();
			...
			if (c.getItems().playerHasItem(995, entryPrice)) {
				...
				for (int j = 0; j < Server.playerHandler.players.length; j++) {
					if (Server.playerHandler.players[j] != null) {
						...
						all.sendMessage("[@blu@Lottery@bla@] "+c.playerName.toUpperCase()+" Has Entered the lottery.");
					}
				}
				//c.sendMessage("@red@You have been entered into the lottery");
				try {
		 			BufferedWriter report = new BufferedWriter(new java.io.FileWriter("./Data/lottery/LotteryEnteries.txt", true));
					try {	
					report.newLine();
	...
	public static void drawLottery() {
		...
		try {
				...
				if(Server.playerHandler.players[i] != null) {
					if(Server.playerHandler.players[i].playerName.equalsIgnoreCase(player)) {
						...
						if (c.getItems().freeSlots() > 0) {
							...
							try {
					 			BufferedWriter report = new BufferedWriter(new java.io.FileWriter("./Data/lottery/LotteryWinners.txt", true));
								try {	
								report.newLine();
								report.write(c.playerName);
								} finally {
							...
							c.sendMessage("You had no space in inventory, "+lotteryFund / 1000 + "k has been added to your bank.");
			...
			for (int j = 0; j < Server.playerHandler.players.length; j++) {
				if (Server.playerHandler.players[j] != null) {
					Client all = (Client)Server.playerHandler.players[j];
					all.sendMessage("[@blu@Lottery@bla@] The Lottery has been won by " + lotteryPlayersNames.get(winner));
				}
			}
		} catch (Exception e) {
			for (int j = 0; j < Server.playerHandler.players.length; j++) {
	...
	public static void announceFund() {
		...
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				Client all = (Client)Server.playerHandler.players[j];
				all.sendMessage("[@blu@Lottery@bla@] The Lottery Jackpot is currently at @blu@" + fund / 1000 + "k@bla@. To Enter speak to Duke Horasio who");
