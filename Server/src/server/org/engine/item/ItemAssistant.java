...
public class ItemAssistant
{
	...
	{
		for( int element: Config.ITEM_TRADEABLE ) {
			if( itemId == element ) {
				return false;
			}
		}
	...
	public void getRequirements( String itemName, int itemId )
	{
		...
		if( itemName.contains( "black" ) ) {
			if( ! itemName.contains( "hide" ) && ! itemName.contains( "knife" ) && ! itemName.contains( "dart" )
	...
	public boolean wearItem( int wearID, int slot )
	{
		...
		synchronized( c ) {
			...
			if( c.playerItems[slot] == wearID + 1 ) {
				...
				/*
				 * Slots: 0 hat 1 cape 2 amulet 3 weapon 4 body 5 shield 6 ? 7
				 * legs 8 ? 9 gloves 10 boots 11 ? 12 ring 13 arrows
				 */
					...
					if( targetSlot == 10 || targetSlot == 7 || targetSlot == 5 || targetSlot == 4 || targetSlot == 0
						...
						if( c.defenceLevelReq > 0 ) {
							if( c.getPA().getLevelForXP( c.playerXP[1] ) < c.defenceLevelReq ) {
								c.sendMessage( "You need a defence level of " + c.defenceLevelReq
										+ " to wear this item." );
								canWearItem = false;
							}
						}
						if( c.runecraftingLevelReq > 0 ) {
							if( c.getPA().getLevelForXP( c.playerXP[20] ) < c.runecraftingLevelReq ) {
								c.sendMessage( "You need a runecrafting level of " + c.runecraftingLevelReq
										+ " to wear this item." );
								canWearItem = false;
							}
						}
					...
					if( targetSlot == 3 ) {
						if( c.attackLevelReq > 0 ) {
							if( c.getPA().getLevelForXP( c.playerXP[0] ) < c.attackLevelReq ) {
								c.sendMessage( "You need an attack level of " + c.attackLevelReq
						...
						if( c.rangeLevelReq > 0 ) {
							if( c.getPA().getLevelForXP( c.playerXP[4] ) < c.rangeLevelReq ) {
								c.sendMessage( "You need a range level of " + c.rangeLevelReq
										+ " to wield this weapon." );
								canWearItem = false;
							}
						}
	...
	/*
	 * public void deleteItem(int id, int amount) { if(id <= 0) return; for (int
	 * j = 0; j < c.playerItems.length; j++) { if (amount <= 0) break; if
	 * (c.playerItems[j] == id+1) { c.playerItems[j] = 0; c.playerItemsN[j] = 0;
	 * amount--; } } resetItems(3214); }
	 */
