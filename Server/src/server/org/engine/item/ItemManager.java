...
public class ItemManager
{
		...
		for( int i = 0; i < c.playerItems.length; i ++ ) {
			if( o != null ) {
				if( c.getItems().tradeable( c.playerItems[i] - 1 ) ) {
					Server.itemHandler.createGroundItem( o, c.playerItems[i] - 1, c.getX(), c.getY(),
					...
					if( c.getItems().specialCase( c.playerItems[i] - 1 ) ) {
						Server.itemHandler.createGroundItem( o, 995, c.getX(), c.getY(),
								c.getItems().getUntradePrice( c.playerItems[i] - 1 ), c.killerId );
					}
					Server.itemHandler.createGroundItem( c, c.playerItems[i] - 1, c.getX(), c.getY(),
			...
			if( o != null ) {
				if( c.getItems().tradeable( c.playerEquipment[e] ) ) {
					...
					if( c.getItems().specialCase( c.playerEquipment[e] ) ) {
						Server.itemHandler.createGroundItem( o, 995, c.getX(), c.getY(),
								c.getItems().getUntradePrice( c.playerEquipment[e] ), c.killerId );
					}
	...
	public static void keepItem( Client c, int keepItem, boolean deleteItem )
	{
		...
		if( itemInInventory ) {
			...
			if( deleteItem ) {
				c.getItems().deleteItem( c.playerItems[slotId] - 1,
						c.getItems().getItemSlot( c.playerItems[slotId] - 1 ), 1 );
			}
		} else {
	...
	public static boolean addItem( Client c, int item, int amount )
	{
		synchronized( c ) {
			...
			if( ( c.getItems().freeSlots() >= 1 || c.getItems().playerHasItem( item, 1 ) ) && Item.itemStackable[item]
					|| c.getItems().freeSlots() > 0 && ! Item.itemStackable[item] ) {
				for( int i = 0; i < c.playerItems.length; i ++ ) {
