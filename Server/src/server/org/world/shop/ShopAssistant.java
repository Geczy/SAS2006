...
public class ShopAssistant
{
	...
	public void buyFromShopPrice( int removeId, int removeSlot )
	{
		...
		if( c.myShopId == 19 || c.myShopId == 20 ) {
			c.sendMessage( c.getItems().getItemName( removeId ) + ": currently costs " + ShopValue + " tokkul"
					+ ShopAdd );
		}
		if( c.myShopId != 19 && c.myShopId != 20 ) {
	...
	public void sellToShopPrice( int removeId, int removeSlot )
	{
		...
		if( IsIn == false ) {
			...
			c.sendMessage( c.getItems().getItemName( removeId ) + ": shop will buy for " + ShopValue + " coins"
					+ ShopAdd );
		}
	}
	...
	public void handleOtherShop( int itemID )
	{
		 ...
		 * 99) { count++; } } return count; } public void setupSkillCapes(int
		 * capes, int capes2) { synchronized(c) { c.getItems().resetItems(3823);
		 * c.isShopping = true; c.myShopId = 14; c.getPA().sendFrame248(3824,
		 * 3822); c.getPA().sendFrame126("Skillcape Shop", 3901); int TotalItems
		 * = 0; TotalItems = capes2; if (TotalItems >
		 ...
		 * c.flushOutStream(); } } public void skillBuy(int item) { int nn =
		 * get99Count(); if (nn > 1) nn = 1; else nn = 0; for (int j = 0; j <
		 * skillCapes.length; j++) { if (skillCapes[j] == item ||
		 * skillCapes[j]+1 == item) { if (c.getItems().freeSlots() > 1) { if
		 ...
		 * c.getItems().resetItems(3823); } public void openVoid() { } public
		 * void buyVoid(int item) { }
		 */
	}
}
